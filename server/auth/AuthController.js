...
function login(req, res, next) {
  ...
  request.post(
    `${config.cdmUrl}customer/IsEmailAddressTaken`,
    { form: checkBody },
    async function(err, IsEmailAddressTakenResponse, IsEmailAddressTakenBody) {
      ...
      if (err) {
        ...
        IsEmailAddressTakenResponse.statusCode == 200 &&
        IsEmailTakenObject.Result == false
      ) {
        // check if the password is valid
        User.findOne(
          { uniqueKey: IsEmailTakenObject.Data.UniqueKey.toLowerCase() },
          {
  ...
  await request.post(
    ...
    async function(err, IsEmailAddressTakenResponse, IsEmailAddressTakenBody) {
      ...
      if (err) {
        ...
        IsEmailAddressTakenResponse.statusCode == 200 &&
        IsEmailTakenObject.Result == true
      ) {
        await request.post(
          `${config.cdmUrl}customer/insertcustomer`,
          { form: insertBody },
          async function(error, insertResponse, insertResponseBody) {
            ...
            if (error) {
              ...
              await User.findOne({ identity: output })
                .sort("-_id")
                .then(key => {
                  if (key) {
                    newkey();
